...
On Ubuntu/Debian, install the CPU try-run dependencies with:
...
config="conf/local/try_cpu.yaml"
...
tokenizer:
  ...
  inter_file_boundary: "\n<|book_boundary|>\n"
  artifacts_dir: "artifacts/local/abc_cpu"
  bpe_vocab_file: ""
  bpe_merges_file: ""
...
tokenization:
  ...
  output_binary: "artifacts/local/abc_cpu/tokenized_dataset.bin"
  chunk_size_mb: 16
...
paths:
  model_file_latest: "checkpoints/local/abc_cpu_latest.ckpt"
  model_file_best: "checkpoints/local/abc_cpu_best.ckpt"
  journal_file: "journal/local/abc_cpu_journal.txt"
...
echo "==> Training one short CPU epoch"
